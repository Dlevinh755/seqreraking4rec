"""
VIP5 Model - Complete implementation from original repository
Source: https://github.com/jeykigung/VIP5/blob/main/src/modeling_vip5.py

This file contains the complete VIP5 model implementation.
Only the adapter imports have been updated to use relative imports.
"""
...
try:
    ...
    try:
        from rerank.models.adapters import (
            AdapterController,
            OutputParallelAdapterLayer
        )
    except ImportError:
        # If adapters not available, set to None (will cause errors if used)
        AdapterController = None
        OutputParallelAdapterLayer = None
        import warnings
        warnings.warn(
            "VIP5 adapters module not found. Please copy adapters/ from VIP5 repo to rerank/models/adapters/"
        )
...
class VisualEmbedding(nn.Module):
    def __init__(self, config):
        ...
        n_vis_tokens = config.n_vis_tokens

        # Visual feature encoding
        clip_project = MLP((feat_dim, (config.d_model * n_vis_tokens) // 2,
                                     config.d_model * n_vis_tokens))
        feat_embedding = [clip_project]
        if self.config.use_vis_layer_norm:
            feat_embedding.append(T5LayerNorm(config.d_model * n_vis_tokens, eps=config.layer_norm_epsilon))
        self.feat_embedding = nn.Sequential(*feat_embedding)

    def forward(self, feats):
        """
        Args
            feats: [B, V_W_L, feat_dim]
        Return
            vis_embedding: [B, V_W_L, n_vis_tokens, d_model]
        """

        B, V_W_L, _ = feats.size()
        feat_embedding = self.feat_embedding(feats) # [B, V_W_L, config.d_model * n_vis_tokens]
        vis_embedding = feat_embedding.reshape(B, V_W_L * self.config.n_vis_tokens, self.config.d_model)
...
class T5LayerFF(nn.Module):
    def __init__(self, config):
        ...
        if config.use_adapter:
            if AdapterController is None:
                raise ImportError("AdapterController not available. Please install VIP5 adapters module.")
...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        ## Set maximum 1024 whole words in a source text
        self.whole_word_embeddings = nn.Embedding(
            1024, config.d_model   ## config.d_model is 512 for T5-small
        )
        ## Reserve maximum 16 categories of tokens (now only use two categories: text & visual)
        self.category_embeddings = nn.Embedding(
            16, config.d_model   ## config.d_model is 512 for T5-small
        )
    ...
    def forward(
        ...
        vis_embeds = self.visual_embedding(vis_feats)
        
        # Merge vis_embeds into inputs_embeds (use category_ids, vis_embeds, inputs_embeds)
        for i in range(B):
            inputs_embeds[i][torch.where(category_ids[i] == 1)[0]] = vis_embeds[i][:len(torch.where(category_ids[i] == 1)[0])]
        
        # Add whole_word_embeds & category_embeds
        ...
        if category_ids is not None:
            category_embeds = self.category_embeddings(category_ids)
            assert category_embeds.shape[-1] == inputs_embeds.shape[-1]
            inputs_embeds = inputs_embeds + category_embeds
...
class VIP5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        if config.use_lm_head_adapter:
            if OutputParallelAdapterLayer is None:
                raise ImportError("OutputParallelAdapterLayer not available. Please install VIP5 adapters module.")
...
@dataclass
class VIP5Seq2SeqLMOutput(ModelOutput):
    """
